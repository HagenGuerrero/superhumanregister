...
A single-page encyclopedia-style app with a filterable specimen grid of heroes, a per-hero dossier (bio, powers, animated power-grid stats, key relationships), and a Correspondence (messages) view for dispatches between the Registrar and cataloged heroes, with quick-reply templates and WhatsApp/Instagram-style quoted replies to a specific message. One thread is reserved for a Gemini-backed AI assistant — its reply is currently a placeholder pending a real backend. Includes a collapsible sidebar and a light/dark theme toggle (persisted to `localStorage`).
...
src/
  ...
  components/
    ...
    MessagesView.tsx     correspondence inbox + thread + reply composer with quick-reply templates
  ...
  data/
    ...
    messages.ts          mock message threads/templates "endpoints" (async stand-ins for a future TS API)
  ...
  types.ts               Hero/House/Stat/Relationship/Message/MessageThread/MessageTemplate/ThreadKind types
